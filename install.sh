...
banner() {
    ...
    echo -e ""
    echo -e "${R}  [!]${W} Author  : ${C}${AUTHOR_NAME}"
    echo -e "${R}  [!]${W} Version : ${Y}v1.0 (Stable)"
    echo -e "${R}  [!]${W} GitHub  : ${W}github.com/${GITHUB_USER}"
    echo -e "${R}  [!]${W} Repo    : ${W}github.com/${GITHUB_USER}/${GITHUB_REPO}"
    echo -e ""
    echo -e "${G}  ==============================================${N}"
    echo -e ""
}
...
do_customize_prompts_menu() {
    ...
    echo -ne "\n${left_pad}${Y}[?]${W} Enter Welcome Banner Text (Default: N-THEME): ${RS}"
    read banner_text
    [ -z "$banner_text" ] && banner_text="N-THEME"

    echo -ne "${left_pad}${Y}[?]${W} Enter Shell Username (Default: NOYON): ${RS}"
    read username
...
EOF
        ...
        local theme_file="$HOME/.oh-my-zsh/themes/n-theme.zsh-theme"
...
font_menu() {
    ...
    printf "\n${left_pad}${C}[${W}04${C}]${G} Caskaydia Cove Nerd Font"
    printf "\n${left_pad}${C}[${W}05${C}]${G} Sauce Code Pro Nerd Font"
